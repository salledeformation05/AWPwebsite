...
const NewsCard: React.FC<NewsCardProps> = ({ article, navigate }) => {
  return (
    <div className="bg-brand-secondary rounded-lg overflow-hidden shadow-lg group flex flex-col">
      <div className="relative h-56 cursor-pointer" onClick={() => navigate('newsArticle', { slug: article.slug })}>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <button onClick={() => navigate('newsArticle', { slug: article.slug })} className="text-brand-accent font-semibold hover:underline self-start text-left">
            Lire la suite &rarr;
        </button>
      </div>
    </div>
